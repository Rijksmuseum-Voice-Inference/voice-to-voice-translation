...
def world_feats(wav_path, sample_rate):
    f0, timeaxis, sp, ap, coded_sp = world_encode_wav(wav_path, fs=sample_rate)
    #normed_coded = normalize_coded_sp(coded_sp, coded_sp.mean(0), coded_sp.std(0))
    np.save(wav_path.replace('.wav', '.npy'), coded_sp, allow_pickle=False)

def extract_spec(wav_path, sample_rate):
    y, sr = librosa.load(wav_path, sr=sample_rate, mono=True)
    D = librosa.stft(y, n_fft=512, win_length=512, hop_length=int(512/2))
    mag, phase = librosa.magphase(D)
    mag = mag[:256, :]
    mag = np.transpose(mag)
    with open(wav_path.replace('.wav', '.spect'), 'wb') as f:
        np.save(f, mag, allow_pickle=False)

def extract_mel_spec(wav_path, sample_rate):
    y, sr = librosa.load(wav_path, sr=sample_rate, mono=True)
    mel_spect = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=40)
    mel_spect = np.transpose(mel_spect)
    with open(wav_path.replace('.wav', '.mel'), 'wb') as f:
        np.save(f, mel_spect, allow_pickle=False)

def load_wav(wav_file, sr):
    wav, _ = librosa.load(wav_file, sr=sr, mono=True)
    return wav

def world_decompose_dio_harvest(wav, fs, frame_period = 5.0):
    # Decompose speech signal into f0, spectral envelope and aperiodicity using WORLD
    wav = wav.astype(np.float64)

    f0_harvest, timeaxis_harvest = pyworld.harvest(wav, fs, frame_period = frame_period, f0_floor = 71.0, f0_ceil = 800.0)
    f0_dio, timeaxis_dio = pyworld.dio(wav, fs, frame_period = frame_period, f0_floor = 71.0, f0_ceil = 800.0)
    f0 = np.max(np.min(f0_harvest, f0_dio), 0)
