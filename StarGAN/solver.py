from vctk_models import Generator
from vctk_models import Discriminator
...
from speech_model import Generator as SpeechG
from speech_model import NoCompressGenerator32, NoCompressGenerator16
from speech_model import UnifiedDiscriminator as SpeechD
from asr.speech_dnn import MLP as asr_mlp
from asr.speech_dnn import load_model as load_asr_mlp
from convert import convert

class Solver(object):
    """Solver for training and testing StarGAN."""

    def __init__(self, allsstar_loader, config):
        """Initialize configurations."""
        # Data loader.
        self.allsstar_loader = allsstar_loader
        ...
        self.d_subset_features = config.d_subset_features
        ...
        self.asr_save_dir = config.asr_save_dir
    ...
    def build_tensorboard(self):
        ...
        self.logger = Logger(self.log_dir, self.exp_name)
    ...
    def classification_loss(self, logit, target, dataset='CelebA'):
        ...
        elif dataset == 'allsstar':
            return F.cross_entropy(logit, target)

    def train(self):
        """Train StarGAN within a single dataset."""
        # Set data loader.
        data_loader = self.allsstar_loader
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if torch.isnan(x_real).sum() > 0:
                print("==> got NaN, skipping batch...")
            ...
            # flip the labels (for binary case)
            #label_trg = 1 - label_org

            if self.dataset == 'CelebA':
                c_org = label_org.clone()
                c_trg = label_trg.clone()
            elif self.dataset == 'RaFD' or self.dataset == 'allsstar':
            ...
            out_src, out_cls = self.D(x_real[:,:,:self.d_subset_features,:])
            #out_src, out_cls = self.D(x_real[:,:,:self.d_subset_features,:])
            #out_src, out_cls = self.D(x_real, c_org)
            #d_loss_real = -torch.mean(torch.log(out_src))
            d_loss_real = -torch.mean(out_src)
            d_loss_cls = self.classification_loss(out_cls, label_org, self.dataset)
            cls_real_accuracy = (out_cls.max(1)[1] == label_org).float().mean()

            # Compute loss with fake images.
            x_fake = self.G(x_real, c_trg)
            out_src, out_cls = self.D(x_fake.detach()[:,:,:self.d_subset_features,:])
            #out_src, out_cls = self.D(x_fake.detach()[:,:,:self.d_subset_features,:])
            #out_src, out_cls = self.D(x_fake.detach(), c_trg)
            #d_loss_fake = -torch.mean(torch.log(1 - out_src))
            d_loss_fake = torch.mean(out_src)

            # Compute loss for gradient penalty.
            alpha = torch.rand(x_real.size(0), 1, 1, 1).to(self.device)
            x_hat = (alpha * x_real.data + (1 - alpha) * x_fake.data).requires_grad_(True)
            out_src, _ = self.D(x_hat[:,:,:self.d_subset_features,:])
            #out_src, _ = self.D(x_hat[:,:,:self.d_subset_features,:])
            #out_src, _ = self.D(x_hat, c_trg)
            d_loss_gp = self.gradient_penalty(out_src, x_hat)

            # Backward and optimize.
            #d_loss = self.lambda_dis * (d_loss_real + d_loss_fake) + self.lambda_cls * d_loss_cls
            d_loss = self.lambda_dis * (d_loss_real + d_loss_fake) + self.lambda_cls * (d_loss_cls) + self.lambda_gp * d_loss_gp
            self.reset_grad()
            d_loss.backward()
            ...
            loss['D/cls_real_acc'] = cls_real_accuracy.item()
            ...
            if (i+1) % self.n_critic == 0:
                ...
                x_fake = self.G(x_real, c_trg)
                out_src, out_cls = self.D(x_fake[:,:,:self.d_subset_features,:])
                #out_src, out_cls = self.D(x_fake[:,:,:self.d_subset_features,:])
                #out_src, out_cls = self.D(x_fake, c_trg)
                #g_loss_fake = -torch.mean(torch.log(out_src))
                g_loss_fake = - torch.mean(out_src)
                g_loss_cls = self.classification_loss(out_cls, label_trg, self.dataset)
                cls_fake_accuracy = (out_cls.max(1)[1] == label_trg).float().mean()
                ...
                # Backward and optimize.
                g_loss = self.lambda_dis * g_loss_fake + self.lambda_rec * (g_loss_rec) + self.lambda_cls * g_loss_cls
                #g_loss = self.lambda_dis * g_loss_fake + self.lambda_rec * (g_loss_rec) + self.lambda_cls * g_loss_cls + kl_div_loss
                self.reset_grad()
                g_loss.backward()
                ...
                #loss['G/loss_kl'] = kl_div_loss.item()
                loss['D/cls_fake_acc'] = cls_fake_accuracy.item()
            ...
            if (i+1) % self.sample_step == 0:
                with torch.no_grad():
                    convert(self.G, "/data/felix/allsstar/toy_16/ALL_094_F_KOR_ENG_DHR_01_9.wav", f"/data/felix/allsstar/samples/ALL_094_F_KOR_ENG_DHR_01_9_recon_0_{i}.wav", c_org[0].unsqueeze(0), self.device, 16000)
                    convert(self.G, "/data/felix/allsstar/toy_16/ALL_094_F_KOR_ENG_DHR_01_9.wav", f"/data/felix/allsstar/samples/ALL_094_F_KOR_ENG_DHR_01_9_recon_1_{i}.wav", c_trg[0].unsqueeze(0), self.device, 16000)
