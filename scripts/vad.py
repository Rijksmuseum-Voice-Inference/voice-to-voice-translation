...
def apply_vad(signal, fs, threshold=0.05, medfilt_kernel=31, window=100):
    """Applies VAD to a given signal

    signal (ndarray): signal array
    fs (int): sampling frequency
    threshold (float): threshold for VAD
    medfilt_kernel (int): size of filter for median filter
    window (int): size of "mistake" window for cropping

    returns a signal (ndarray) cropped according to VAD
    """
    detector = VAD(fs)
    speech = detector.detect_speech(signal, threshold)  # detect speech
    speech_smooth = medfilt(speech, kernel_size=medfilt_kernel)  # median filter smoothing
    speech_idx = np.argwhere(speech_smooth > 0)  # find indexes where there is speech
    start, end = speech_idx[0], speech_idx[-1]  # get start and end
    start, end = start[0], end[0]  # convert from list to scalar
    ratio = signal.shape[0] / speech.shape[0]  # get ratio between len of signal and len of speech (in frames)
    start, end = start * ratio, end * ratio  # get start and end in signal frequency
    start, end = max(0, start - window), min(signal.shape[0]-1, end + window)  # "mistake" window
